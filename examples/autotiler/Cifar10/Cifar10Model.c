...
void Cifar10Model(unsigned int L1Memory)
{
    ...
    CNN_ConvolutionPoolReLU("Conv5x5MaxPool2x2_SW_0", 0, 2, 2, 2, 2, 12,14,14,12, 0, 0, 0, 0, 1, 8, 32, 32, KOP_CONV, 5, 5, 1, 1, 1, 1, 0, KOP_MAXPOOL, 2, 2, 1, 1, 2, 2, 1, KOP_NONE);
    // 5x5 Convolution followed by 2x2 Max pooling. Pure SW.
    // 8 input planes [14x14], 12 output planes [5x5]
    CNN_ConvolutionPoolReLU("Conv5x5MaxPool2x2_SW_1", 0, 2, 2, 2, 2, 12,14,14,12, 0, 0, 0, 0, 8, 12, 14, 14, KOP_CONV, 5, 5, 1, 1, 1, 1, 0, KOP_MAXPOOL, 2, 2, 1, 1, 2, 2, 1, KOP_NONE);
    // Linear Layer
    // Input 12 x [5x5], Output 10
    CNN_LinearReLU("LinearLayerReLU_1", 0, 2, 2, 2, 2, 12,14,10,16, 0, 0, 0, 0, 12*5*5, 10, KOP_LINEAR, KOP_NONE);
}
